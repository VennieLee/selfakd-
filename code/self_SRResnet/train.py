...
import utility
import math
from feature_transformation import spatial_similarity
import imageio
...
def create_student_model():
    ...
    if args.model == 'SelfSRRes':
        args.n_resblocks = 16
        args.n_feats = 64
        args.res_scale = 1.0
        student = self_srresnet.SelfSRRes(args).to(device)    
...
def train(epoch):
    ...
    for batch, (lr, hr, idx_scale) in enumerate(train_loader):
        ...
        output, middle_output1, middle_feature1, middle_output2, middle_feature2, \
        middle_output3, middle_feature3, final_feature = student(lr)
       
        total_loss = criterion(hr, output, middle_output1, middle_feature1, middle_output2, middle_feature2, \
        middle_output3, middle_feature3, final_feature)    
        total_loss.backward()
        optimizer.step()
        ...
        for idx_data, d in enumerate(test_loader):
            for idx_scale, scale in enumerate(args.scale):
                ...
                for lr, hr, filename in tqdm(d, ncols=80):
                    lr, hr = prepare(lr, hr)
                    sr, middle_output1, middle_feature1, middle_output2, middle_feature2, \
                    middle_output3, middle_feature3, final_feature = student(lr)
