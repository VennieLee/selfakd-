...
sys.path.append(os.path.dirname(os.path.dirname('D:/fakd/code/fakd_SRResnet/train.py')))
import data
...
def load_teachers():
    ...
    if "FakdSRRes" in args.teacher:
        args.n_resblocks = 32
        args.n_feats = 64
        net = fakd_srres.FakdSRRes(args).to(device)
        if int(args.scale[0]) == 4:
            print("loading SRResx4")
            net.load_state_dict_teacher(torch.load('D:/fakd/teacher_checkpoint/SRRes_BIX4.pt'))
...
def create_student_model():
    ...
    elif args.model == 'FakdSRRes':
        args.n_resblocks = 16
        args.n_feats = 64
        student = fakd_srres.FakdSRRes(args).to(device)
    if args.precision == 'half':
        student.half()
    if args.resume:
